const drawBoard = () => {
  ...
  for (let j = 0; j < 4; j++) {
    drawLine((width / 2) - (height / 2 - height / 10), height / 3 * j + height / 9,(width / 2) - (height / 2 - height / 10), height / 3 * j + height / 9, 'black', 3)
  }
}
drawBoard()
  ...
  for (let j = 0; j < board.length; j++)
    for (let i = 0; i < 9; i++) {
      if (board[j][i] !== '') {
        ...
        drawText(board[j][i], width / 9 * (i), height / 9 * (j + 1), 'black', 25);
      }
    }
}
drawArray()
